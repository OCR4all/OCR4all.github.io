<template><div class="custom-container danger"><p class="custom-container-title">Achtung</p>
...
<p>V. a. Erstnutzern wird an dieser Stelle ohnehin geraten, mindestens einmal den nun folgenden schrittweisen Workflow der OCR durchzuführen, um die Funktionsweise der jeweiligen Teilmodule zu verstehen.</p>
<h3 id="_4-2preprocessing" tabindex="-1"><a class="header-anchor" href="#_4-2preprocessing" aria-hidden="true">#</a> 4.2	Preprocessing</h3>
<p><strong>Input:</strong> Originalbild (farbig, in Graustufen oder binär)<br>
<strong>Output:</strong> entzerrtes Binär- (und Graustufen-)Bild</p>
<ul>
...
<li>Ob die Binarisierung erfolgreich war, kann unter „Project Overview” und durch den Klick auf einen beliebigen „Page Identifier” sowie die Bildanzeige „Binary” kontrolliert werden. Außerdem sollten in der Projektübersicht in der Spalte “Preprocessing” für alle bearbeiteten Bilddateien Haken erschienen sein.</li>
</ul>
<h3 id="_4-3noise-removal" tabindex="-1"><a class="header-anchor" href="#_4-3noise-removal" aria-hidden="true">#</a> 4.3	Noise Removal</h3>
<p><strong>Input:</strong> verunreinigte Binärbilder<br>
<strong>Output:</strong> Binärbilder ohne oder mit nur wenigen Verunreinigungen</p>
<ul>
<li>Mit Hilfe der Option Noise Removal können bspw. kleinere Verunreinigungen wie Flecken und Punkte auf den Scans getilgt werden.</li>
...
<li>Wurden zu viele Bildelemente entfernt, korrigieren Sie den Wert der „Maximal size for removing contours” nach unten.</li>
<li>Verfahren Sie in dieser Weise weiter, bis Sie mit dem Ergebnis zufrieden sind.</li>
</ul>
...
<ul>
<li>Ausrufezeichen, orange: „There is no segmentation for this page.“ – Aktuell liegen keine Segmentierungsergebnisse für diese Scanseite vor.</li>
<li>Warndreieck, orange: „Current segmentation may be unsaved.“ – Die aktuellen Segmentierungsergebnisse sind noch nicht gespeichert (s. u.).</li>
...
<li><strong>Image Zoom</strong>: Über die hier möglichen Einstellungen wird die allgemeine Darstellung von Scanseiten und Bilddateien in LAREX geregelt, d. h. z. B. Zoomeinstellungen. Allerdings können diese Einstellungen und Darstellungsoptionen auch mithilfe der Maus und/oder des Touchpads geregelt werden (einfaches Verschieben der angezeigten Seite durch gehaltenen Linksklick auf den Scan und Bewegung der Maus; Zoom über Mausrad oder Zoomeinstellungen des Touchpads).</li>
<li><em>BILDCHEN</em> <strong>Undo</strong> und <em>BILDCHEN</em> <strong>Redo</strong>:    Zurücknehmen    oder    Wiederholen    der     letzten Benutzeraktion. Auch übliche Tastenkombinationen sind hier möglich (z. B.: STRG + Z
= letzte Aktion rückgängig machen).</li>
...
<h4 id="_4-4-4beispielhafte-segmentierung-einer-scanseite" tabindex="-1"><a class="header-anchor" href="#_4-4-4beispielhafte-segmentierung-einer-scanseite" aria-hidden="true">#</a> 4.4.4	Beispielhafte Segmentierung einer Scanseite</h4>
...
<p>Darüber hinaus bestehen generell <strong>weitere Bearbeitungsmöglichkeiten</strong> von Scans, die im Folgenden angezeigt werden sollen:</p>
<ul>
<li>Für Löschungen oder die Zusammenführung mehrere Layoutelemente zu einer zusammenhängenden Region ist es praktisch, diese <strong>gleichzeitig auswählen</strong> zu können. Dazu halten Sie die Umschalttaste gedrückt und ziehen mit Hilfe der Maus ein Rechteck um die entsprechenden Layoutregionen. Die Regionen müssen sich dazu vollständig innerhalb des Rechtecks befinden. Alle auf diese Weise ausgewählten Layoutregionen erscheinen nun blau umrandet.</li>
<li><strong>„Select contours to combine (with „C“) to segments (see function combine)“</strong> (Shortcut: 6): Dieses Werkzeug kann verwendet werden, um auch auf sehr eng und detailreich bedruckten Seiten zu einem optimalen Segmentierungsergebnis zu gelangen. Grundlegende Idee ist, dass Layoutelemente durch die Konturen der einzelnen Typen des Textes, den sie beinhalten, oder exakt durch die Ränder von Bildern und Zierinitialen begrenzt werden – ohne überschüssigen, durch händisches Segmentieren entstehenden Rand, der immer wieder zur Elementüberschneidungen und damit zu Ungenauigkeiten mit Folgen für die OCR führen kann.
<ul>
<li>Um die Funktion auszuführen, erfolgt zuerst ein Klick auf den entsprechenden Button in der Werkzeugleiste oder der Shortcut 6. Daraufhin werden alle als Layoutelemente der Seite erkannten Bestandteile blau eingefärbt.</li>
</ul>
</li>
</ul>
...
<li><strong>„Fix/unfix segments, for it to persist a new auto segmentation“</strong> (Shortcut: F): Mit Hilfe dieser Funktion können Layoutelemente über einen weiteren Segmentierungsvorgang einer Seite hinaus fixiert werden. Dazu wird das entsprechende Layoutelement durch Anklicken markiert, danach folgt ein Klick auf „F“ oder den entsprechenden Button. Fixierte Elemente erscheinend mit einer gestrichelten Umrandung. Um die Fixierung zu verwerfen, wird der Vorgang einfach wiederholt.</li>
...
<li><strong>„LOAD RESULTS“</strong>: Mit Hilfe dieser Funktion können bereits bestehende Segmentierungsergebnisse für eine bestimmte Scanseite direkt aus der Ordnerstruktur von OCR4all in LAREX geladen werden.</li>
</ul>
<h4 id="_4-4-6abschluss-der-segmentierung-mit-larex" tabindex="-1"><a class="header-anchor" href="#_4-4-6abschluss-der-segmentierung-mit-larex" aria-hidden="true">#</a> 4.4.6	Abschluss der Segmentierung mit LAREX</h4>
<ul>
<li>Sind alle Segmentierungsarbeiten für ein Werk in LAREX abgeschlossen, d. h. wurden für jede Seite eines Werks Ergebnisse abgespeichert, so liegen diese nun in der bekannten Ordnerstruktur von OCR4all vor.</li>
<li>Ob die Segmentierung und Speicherung der Ergebnisse erfolgreich war, kann danach abschließend im Menüpunkt „Post Correction“ in der Spalte „SEGMENTS“ kontrolliert werden (s. u.).</li>
</ul>
...
<p><strong>Input</strong>: vorverarbeitete Bilder und Segmentierungsinformationen in Form von PageXML- Dateien<br>
<strong>Output</strong>: extrahierte Textzeilen in den PageXML-Dateien</p>
<ul>
...
<ul>
<li>Zur Erklärung: „<strong>Whitespace column separators</strong>” sind spaltenweise gesehen die weißen Randbereiche um Textblöcke.</li>
<li>Bei einem <strong>zweispaltigen Layout</strong>, dessen Text inhaltlich fortlaufend ist, d. h. die jeweils ersten Zeilen der beiden Textblöcke keine inhaltliche Einheit bilden, muss der Wert bei „Maximum # of whitespace column separators” auf 3 gesetzt werden: Diese Angabe ergibt sich aus dem linken whitespace der linken Textspalte, dem rechten whitespace der rechten Textspalte sowie dem gemeinsamen whitespace zwischen beiden Textspalten.</li>
<li>Bei einem <strong>dreispaltigen Layout</strong> müsste der Wert entsprechend auf 4 verändert werden usw.</li>
</ul>
</li>
<li>Sobald alle Einstellungen wie gewünscht getroffen sind, klicken Sie auf „EXECUTE” und überprüfen die Ergebnisse abermals unter „Project Overview”. Hier erhalten die einzelnen Zeilen als Unterpunkte der einzelnen Layoutelemente (s. o.).</li>
<li>Vor allem bei der Line Segmentation ist immer wieder die Anwendung der erweiterten Einstellungen („Settings (Advanced)”) hilfreich – v. a. dann, wenn in der Konsole Fehlermeldungen angezeigt werden und die Zeilensegmentierung entsprechend nicht fehlerfrei durchgeführt werden konnte. Beispielsweise wird bei zu kleinen Buchstaben häufig die in den Defaults festgehaltene Minimalbreite von ganzen Zeilen unterschritten. Diese Minimalbreite kann jedoch bspw. durch die Herabsetzung des Wertes „Minimum scale permitted” unter dem Menüpunkt „Limits” geändert werden. Die wiederholte Durchführung der Line Segmentation für die ausgewählten Scanseiten wird dann ohne Fehlermeldung korrekt vollzogen.</li>
<li>Überprüfbar ist die korrekte Zeilensegmentierung auch unter dem Menüpunkt „Post Correction” im Reiter „Lines“ (s. u.).</li>
</ul>
<h3 id="_4-6recognition" tabindex="-1"><a class="header-anchor" href="#_4-6recognition" aria-hidden="true">#</a> 4.6	Recognition</h3>
<p><strong>Input</strong>: Textzeilen und ein oder mehrere OCR-Modelle<br>
<strong>Output</strong>: OCR-Output in Textform für jede in den PageXML-Dateien vorliegenden Zeile</p>
<ul>
<li>Der Arbeitsschritt der Recognition stellt die Erkennung von Text auf Grundlage der während der Line Segmentation (s. o.) erstellten Zeilenbilder aller Layoutelemente mit Text dar.</li>
...
<li>Klicken Sie nun auf „EXECUTE” und warten Sie die Texterkennung über die Fortschrittsanzeige und die Konsole ab.</li>
<li>Ist die Erkennung abgeschlossen, können Sie die Ergebnisse für jedes Zeilenbild unter dem Menüpunkt „Ground Truth Production” einsehen.</li>
</ul>
...
<p><strong>Input</strong>: Zeilenbild und entsprechender OCR-Output, wenn verfügbar<br>
<strong>Output</strong>: zeilenbasierte Ground Truth</p>
<ul>
...
<p><strong>Input</strong>: Zeilenbilder mit entsprechender Ground Truth sowie optional bereits bestehende OCR-Modelle, die als sog. Pretraining und Datengrundlage des Modelltrainings genutzt werden<br>
<strong>Output</strong>: ein oder mehrere OCR-Modelle</p>
<p>Generell muss es das Ziel sein, einen insgesamt möglichst fehlerfreien Text zu erhalten!<br>
Warum dann aber die Erstellung werkspezifischer Modelle mittels des Trainings-Moduls statt einfacher, abschließender Textkorrektur?</p>
<p>→ Je besser das Modell, welches zur Texterkennung genutzt wird, desto kürzere fällt die Korrekturzeit aus. Idee und Sinn eines kontinuierlichen Modelltrainings sind es also, mit fortlaufendem Korrekturfortschritt auch immer bessere Modelle zu trainieren und somit den Korrekturaufwand für die danach noch zu korrigierenden Seiten des Werkes auf ein Minimum zu reduzieren.</p>
<ul>
<li>Innerhalb des Trainingstools können auf Grundlage aller zu einem Werk vorliegenden Zeilen Ground Truth werkspezifische Modelle bzw. -ensembles trainiert werden. Dazu werden in den allgemeinen Einstellungen folgende Werte eingetragen:
<ul>
<li>„<strong>The number of folds</strong> (= the number of models) <strong>to train</strong>“: <strong>5</strong> → Es wird im Folgenden ein Modellensemble, bestehend aus fünf Einzelmodellen, trainiert.</li>
<li>„<strong>Only train a single fold</strong> (= a single model)“: <em>Nichts eintragen!</em> → Es werden alle fünf Einzelmodelle statt nur eines einzelnen trainiert.</li>
<li>„<strong>Number of models to train in parallel</strong>“: <strong>-1</strong> → Alle Modelle des Ensembles werden gleichzeitig trainiert.</li>
<li>„<strong>Keep all characters loaded from the last model</strong>”: Auswählen, wenn alle in den „Pretraining”-Modellen enthaltenen Zeichen im zu trainierenden Modell beibehalten werden sollen, also zu dessen Whitelist hinzugefügt werden.</li>
<li>„<strong>Whitelist characters to keep in the model</strong>”: Liste von Zeichen, die beim Training und im daraus entstehenden Modell berücksichtigt werden. Alle Zeichen außerhalb dieser „Whitelist“ werden nicht berücksichtigt.</li>
<li>„Pretraining“:<br>
„<strong>Train each model based on different existing models</strong>“ (Im Folgenden öffnen sich fünf Dropdown-Listen; in jede wird eines der gemischten Modelle des Modellensembles eingetragen, das wie empfohlen zur ersten Erkennung von Text im vorliegenden Werk genutzt wurde; egal bei welcher Trainingsiteration der Nutzer steht: Auch wenn bspw. bereits das dritte werkspezifische Modell trainiert wird – es werden trotzdem immer die fünf zu Beginn verwendeten grundlegenden gemischten Modelle eingetragen)<br>
<strong>ODER</strong><br>
„<strong>Train all models based on one existing model</strong>“ (Wurde die erste Texterkennung auf Grundlage eines einzelnen gemischten Modells durchgeführt, so wird nur ein Modell eingetragen; jedoch gilt auch hier, dass bei jeder Iteration eben dieses gemischte Modell erneut angegeben werden muss).</li>
<li>„<strong>Data augmentation</strong>“: <em>Nichts eintragen.</em> → Aber: beschreibt die Anzahl der Datenerweiterungen pro Zeile. Es kann hier ein Wert, bspw. 5, angegeben werden, um damit die Menge des Trainingsmaterials zu erhöhen, auf der trainiert wird. Dies kann zur Erstellung besserer Modelle führen, benötigt aber deutlich mehr Trainingszeit.</li>
<li>„<strong>Skip retraining on real data only</strong>“: <em>Nicht auswählen!</em></li>
</ul>
</li>
...
<p><strong>Input</strong>: Segmentierungsinformationen für vorverarbeitete Bilder und zugehöriger Text<br>
<strong>Output</strong>: Korrigierte Segmentierungsinformationen und Text</p>
<p>Unter dem Menüpunkt „Post Correction“ können die in den vorherigen Teilmodulen erstellten Segmentierungsinformationen und Texte manuell angepasst und korrigiert werden. Das Teilmodul ist hierbei in drei Ebenen untergliedert:</p>
<ul>
...
<li>Unter „Settings” kann ausgewählt werden, ob Text- oder PageXML-Dateien erstellt werden sollen. Im Falle der Text-Dateien wird sowohl für jede Scanseite eine einzelne TXT erstellt, als auch eine zusammenhängende, die den Gesamttext des bearbeiteten Werks beinhaltende ausgegeben.</li>
<li>Die PageXML-Dateien werden auf Scanseitenbasis ausgegeben und enthalten Angaben zum Erstellungsdatum, zu letzten Dateiänderungen, zu Metadaten der sich auf sie beziehenden Scanseite, zur Seitengröße, zu auf der Seite enthaltenen Layoutelementen inklusive deren genaue Koordinaten, zur Reading Order der vorhandenen Layoutelemente, zu den einzelnen Textzeilen sowie den Text der Zeilen selbst.</li>
</ul>
</template>
